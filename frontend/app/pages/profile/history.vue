...
<template>
    ...
    >
        ...
        <div class="rounded-xl border border-gray-100 dark:border-[#232323] overflow-hidden dark:bg-[#171717] mt-6">
        ...
        <DashboardCard >
            <div class="flex items-center justify-between mb-6">
              <h1 class="text-xl font-bold text-gray-900 dark:text-white/80">
                {{ t("nav.chargingHistory") }}
              </h1>
              <span class="text-xs text-gray-500 dark:text-white/40">
                {{ chargingHistory.length }} {{ chargingHistory.length === 1 ? "session" : "sessions" }}
              </span>
            </div>

          <div v-if="chargingHistory.length === 0" class="text-center py-20">
            <p class="text-sm text-gray-400 dark:text-white/30">No charging history yet.</p>
